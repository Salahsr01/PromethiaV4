...
import { useDynamicDashboard } from '../contexts/DynamicDashboardContext'
...
export function DynamicMiniChat() {
  ...
  const handleSendMessage = async () => {
    if (inputValue.trim() && !isLoading) {
      ...
      try {
        // Appeler l'agent avec le contexte des widgets dynamiques
        ...
        if (response.ok) {
          ...
          if (success && modification && modification.type !== 'none' && modification.config) {
            // Pour les dashboards dynamiques, on peut mettre à jour un widget spécifique
            // ou l'ensemble des widgets selon la modification
            if (modification.widgetIndex !== undefined) {
              const updatedWidgets = [...widgets]
              updatedWidgets[modification.widgetIndex] = {
                ...updatedWidgets[modification.widgetIndex],
                ...modification.config
              }
              setWidgets(updatedWidgets)
            } else if (modification.widgets) {
              setWidgets(modification.widgets)
            }
          }
  ...
  return (
    <div className="relative w-full h-full flex flex-col">
                    ...
                    {msg.modification && msg.modification.type !== 'none' && (
                      <div className="mt-1 flex items-center gap-1.5">
                        ...
                        <span className="text-green-400 text-[10px]">
                          ✓ {msg.modification.description || `Widget modifié`}
                        </span>
                      </div>
                    )}
                  </div>
      ...
      <div className="px-[11px] pb-[11px]">
        {/* Badge de contexte du widget sélectionné */}
        {selectedWidget && (
        ...
        <div className="relative">
          <input
            ...
            placeholder={selectedWidget ? `Demandez quelque chose sur ${selectedWidget}...` : "Ecrivez votre message"}
